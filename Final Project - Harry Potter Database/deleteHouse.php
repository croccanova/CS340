...
<?php
	...
	// Deletes house selected from drop down menu
	if (!($stmt = $mysqli->prepare("DELETE FROM house WHERE house.id = ?")))
	{
		echo "Prepare failed: " . $stmt->connect_errno . " " . $stmt->connect_error;
	}
	if (!($stmt->bind_param("i", $_POST['houseName']))){
	...
	{
		echo "Deleted " . $stmt->affected_rows . "row(s) from house.";
	}
?>
