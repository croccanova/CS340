...
<?php
	...
	// Deletes patronus selected from drop down menu
	if (!($stmt = $mysqli->prepare("DELETE FROM patronus WHERE patronus.id = ?")))
	{
		echo "Prepare failed: " . $stmt->connect_errno . " " . $stmt->connect_error;
	}
	if (!($stmt->bind_param("i", $_POST['delPatronus']))){
	...
	{
		echo "Deleted " . $stmt->affected_rows . "row(s) from patronus.";
	}
?>
